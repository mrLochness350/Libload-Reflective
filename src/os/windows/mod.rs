...
impl ReflectedLibrary {
    ...
    fn write_sections(
        ...
        if e_lfanew >= buffer.len() {
            return Err(ReflectError::GenericError {
                fmt: "Invalid e_lfanew value".to_string(),
            });
        }
    
        let mut section_header_start = (base_ptr as usize + e_lfanew + NT_HEADER_SIZE) as *const IMAGE_SECTION_HEADER;
        if section_header_start.is_null() {
            return Err(ReflectError::GenericError {
                fmt: "Null section header start pointer".to_string(),
            });
        }
        ...
        for _ in 0..num_of_sections {
            unsafe {
                ...
                if raw_data_start + raw_data_size > buffer.len() {
                    continue;
                }
                let dest_ptr = (base_ptr as usize + virtual_address) as *mut c_void;
                if dest_ptr.is_null() {
                    
                    continue;
                }
    
                if let Some(section_data) = buffer.get(raw_data_start..raw_data_start + raw_data_size) {
    ...
    fn fix_base_relocations(
        ...
        #[cfg(target_arch = "x86_64")]
        let optional_header = &(*(nt_header as *const IMAGE_NT_HEADERS64)).OptionalHeader ;
        #[cfg(target_arch = "x86")]
        let optional_header = &(*(nt_header as *const IMAGE_NT_HEADERS32)).OptionalHeader ;
        
        let base_reloc = &optional_header.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC.0 as usize];
        
        if base_reloc.Size == 0 {
            
            return Err(ReflectError::GenericError {
                fmt: "Invalid base relocation size".to_string(),
            });
        }
        
        let image_base = optional_header.ImageBase;
        
        if (base_ptr as usize) < (image_base as usize) {
            
            return Err(ReflectError::GenericError {
                fmt: "Potential underflow detected".to_string(),
            });
        }
        let delta = (base_ptr as usize).wrapping_sub(image_base as usize);
        
        let mut reloc_ptr = (base_ptr as usize + base_reloc.VirtualAddress as usize)
            as *const IMAGE_BASE_RELOCATION;
    
            while !reloc_ptr.is_null() && (*reloc_ptr).SizeOfBlock != 0 {
                let block_size = (*reloc_ptr).SizeOfBlock as usize;
                let entries = (block_size - core::mem::size_of::<IMAGE_BASE_RELOCATION>()) / 2;
    
                let reloc_block_start = (reloc_ptr as usize + core::mem::size_of::<IMAGE_BASE_RELOCATION>()) as *const u8;
    
                for i in 0..entries {
                    let reloc_offset_ptr = reloc_block_start.add(i * 2) as *const u16;
    
                    if reloc_offset_ptr.is_null() {
                        return Err(ReflectError::GenericError {
                            fmt: "Null relocation offset pointer".to_string(),
                        });
                    }
    
                    let current_reloc_offset = core::ptr::read_unaligned(reloc_offset_ptr);
                    if (current_reloc_offset >> 12) != IMAGE_REL_BASED_ABSOLUTE as u16 {
                        ...
                        if final_address as *const usize == std::ptr::null() {
                            return Err(ReflectError::GenericError {
                                fmt: "Null final relocation address".to_string(),
                            });
                        }
    
                        let original_address = core::ptr::read_unaligned(final_address as *const usize);
                        let fixed_address = original_address.wrapping_add(delta);
    
                        core::ptr::write_unaligned(final_address as *mut usize, fixed_address);
                    }
                }
    
                reloc_ptr = (reloc_ptr as *const u8).add(block_size) as *const IMAGE_BASE_RELOCATION;
            }
        }
    ...
    fn fix_import_table(
        ...
        unsafe {
            let size_of_image = (*(nt_header as *const IMAGE_NT_HEADERS64))
                .OptionalHeader
                .SizeOfImage as usize;
            while !original_first_thunk_ptr.is_null()
                && (*original_first_thunk_ptr).Name != 0
                && (*original_first_thunk_ptr).FirstThunk != 0
            {
                let import_descriptor = read_unaligned(original_first_thunk_ptr);
                let name_ptr = (base_ptr as usize + import_descriptor.Name as usize) as *const u8;
                if name_ptr.is_null() || name_ptr as usize >= base_ptr as usize + size_of_image {
                    
                    return Err(ReflectError::GenericError {
                        fmt: "Invalid name pointer".to_string(),
                    });
                }

                let dll_name = self.read_string_from_memory(name_ptr);
                let trimmed_dll_name = dll_name
                    .as_bytes()
                    .split(|&b| b == 0)
                    .next()
                    .unwrap_or_default();
                ...
                let dll_handle = match LoadLibraryA(PCSTR::from_raw(dll_name_cow.as_ptr().cast())) {
                    Ok(h) => h,
                    Err(e) => {
                ...
                while read_unaligned(thunk_ptr as *const usize) != 0 {
                    ...
                    if func_name_ptr.is_null()
                        || func_name_ptr as usize >= base_ptr as usize + size_of_image
                    {
                        
                        return Err(ReflectError::GenericError {
                            fmt: "Invalid function name pointer".to_string(),
                        });
                    }

                    let func_name = self.read_string_from_memory(func_name_ptr);
                    if let Some(address) =
                        GetProcAddress(dll_handle, PCSTR::from_raw(func_name.as_bytes().as_ptr()))
                    {
    ...
    pub unsafe fn new(mut buffer: Vec<u8>) -> Result<Self, ReflectError> {
        let module_base = buffer.as_mut_ptr();
        ...
        match lib.write_sections(base_ptr, buffer, nt_header, dos_header) {
            Ok(()) => {},
            Err(e) => {
        ...
        match lib.fix_base_relocations(base_ptr, nt_header) {
            Ok(()) => {},
            Err(e) => {
        ...
        match lib.fix_import_table(base_ptr, nt_header) {
            Ok(()) => {},
            Err(e) => {
                return Err(e);
            }
